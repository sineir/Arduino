...
void setup() {
  // put your setup code here, to run once:
    ...
    Serialx.println(((F_CPU / F_INTERRUPTS)/8) - 1);
    Serialx.println(F_CPU);
    Serialx.println(F_INTERRUPTS);
}
...
void loop() {
  // put your main code here, to run repeatedly:
    //led();
    digitalWrite(PIN_LED, HIGH);delay(tempo);
    digitalWrite(PIN_LED, LOW);delay(tempo);
    
    if (irmp_get_data (&irmp_data))
    {
            // ir signal decoded, do something here...
            // irmp_data.protocol is the protocol, see irmp.h
            Serialx.print("Protocol: ");Serialx.println(irmp_data.protocol);
            // irmp_data.address is the address/manufacturer code of ir sender
            // irmp_data.command is the command code
            Serialx.print("Data: ");Serialx.println(irmp_data.command);
            if (irmp_data.command == 12) tempo=100;
            if (irmp_data.command == 28) tempo=500;
            if (irmp_data.command == 74) tempo=1000;
            // irmp_protocol_names[irmp_data.protocol] is the protocol name (if enabled, see irmpconfig.h)
            Serialx.print("Name: ");Serialx.println(irmp_protocol_names[irmp_data.protocol]);
            Serialx.println();
     }
}
